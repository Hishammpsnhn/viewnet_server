...
import {
  SeriesEntity,
  SeriesMeta,
} from "../../../domain/entities/series/seriesEntity";
...
export class SeriesRepository implements ISeriesRepository {
  ...
  async searchQuery(query: string): Promise<SeriesMeta[]> {
    const seriesData = await SeriesModel.find({
      title: { $regex: query, $options: "i" },
      isBlock: false,
      isRelease: true,
    })
      .select("title description genre posterImage")
      .limit(10);
    const series = seriesData.map((series) => {
  ...
  async findById(id: string, userId?: string): Promise<SeriesEntity> {
    try {
      ...
      const progressData = await ProgressModel.find({
        profileId: userId,
        videoCatalogId: {
          $in: series.seasons.flatMap((season) =>
      ...
      const progressMap = new Map(
        progressData.map((progress) => [
          progress.videoCatalogId.toString(),
          progress.progress,
        ])
      );
